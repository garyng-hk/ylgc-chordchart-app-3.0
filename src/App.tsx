...
function App() {
  ...
  // State for the new diagnostics tool
  const [driveTestStatus, setDriveTestStatus] = useState<'idle' | 'testing' | 'ok' | 'error'>('idle');
  ...
  // Handler for the new diagnostics test
  const handleDriveTest = useCallback(async () => {
    ...
    try {
        ...
        if (response.ok && data.status === 'ok') {
            setDriveTestStatus('ok');
        } else {
            setDriveTestStatus('error');
        }
        setDriveTestMessage(data.message || `測試失敗，伺服器回應碼: ${response.status}`);
    } catch (err) {
        setDriveTestStatus('error');
        setDriveTestMessage(err instanceof Error ? err.message : '無法呼叫 /api/test-drive 端點。');
    }
  }, []);
    ...
    <>
        <div className="min-h-screen bg-slate-900 font-sans flex flex-col items-center p-4 sm:p-6">
            ...
            <main className="w-full max-w-5xl flex-grow">
                ...
                {/* Diagnostics Panel */}
                {serverStatus === 'ok' && (
                    <div className="mt-4 bg-slate-800/50 rounded-xl p-4 border border-slate-700 text-sm animate-fade-in">
                        <div className="flex items-center justify-between gap-4">
                            <div>
                                <h4 className="font-semibold text-slate-200">診斷工具</h4>
                                <p className="text-slate-400">基本伺服器連線正常。請手動測試與 Google Drive API 的連線。</p>
                            </div>
                            <button
                                onClick={handleDriveTest}
                                disabled={driveTestStatus === 'testing'}
                                className="px-3 py-2 bg-sky-600 text-white font-semibold rounded-md hover:bg-sky-500 disabled:bg-slate-600 disabled:cursor-wait transition-colors flex-shrink-0"
                            >
                                {driveTestStatus === 'testing' ? '測試中...' : '測試 Drive 連線'}
                            </button>
                        </div>
                        {driveTestStatus !== 'idle' && (
                            <div className="mt-3 pt-3 border-t border-slate-700">
                                <p className="flex items-center">
                                  <span className="font-semibold text-slate-300 mr-2">測試結果:</span>
                                  {driveTestStatus === 'ok' && <span className="text-green-400 font-bold">成功</span>}
                                  {driveTestStatus === 'error' && <span className="text-red-400 font-bold">失敗</span>}
                                  {driveTestStatus === 'testing' && <span className="text-yellow-400 font-bold">執行中...</span>}
                                </p>
                                <p className="text-slate-500 text-xs mt-1 break-words">{driveTestMessage}</p>
                            </div>
                        )}
                        <style>{`
                            @keyframes fade-in {
