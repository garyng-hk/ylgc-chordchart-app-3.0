...
const ServerStatusIndicator: React.FC<{ status: 'checking' | 'ok' | 'error', errorMessage?: string }> = ({ status, errorMessage }) => {
    const statusConfig = {
        ...
        error: { color: 'bg-red-500', text: '伺服器連線或設定失敗' },
    };

    const titleText = status === 'error' ? `錯誤: ${errorMessage}` : statusConfig[status].text;

    return (
        <div className="fixed bottom-4 right-4 flex items-center bg-slate-800/80 backdrop-blur-sm p-2 rounded-full shadow-lg text-xs text-slate-300 group" title={titleText}>
            <div className={`w-3 h-3 rounded-full mr-2 ${statusConfig[status].color} ${status !== 'error' ? 'animate-pulse-fast' : ''}`} />
...
function App() {
    ...
    const checkServerStatus = async () => {
      try {
        ...
        if (data.status === 'ok') {
          ...
          const specificError = data.message || '伺服器回報了未知的設定錯誤。';
          setServerErrorMessage(specificError);
          setError(specificError); // Display the specific error from the server immediately
        }
      } catch (err) {
        setServerStatus('error');
  ...
  const handleSearch = useCallback(async (params: { songTitle: string; key: string; lyrics: string; }) => {
    if (serverStatus !== 'ok') {
        setError(serverErrorMessage || '無法執行搜尋，因為伺服器連線或設定失敗。');
        return;
    }
        ...
        <div className="min-h-screen bg-slate-900 font-sans flex flex-col items-center p-4 sm:p-6">
            ...
            <main className="w-full max-w-5xl flex-grow">
                <div className="bg-slate-800/50 backdrop-blur-sm rounded-xl p-6 shadow-2xl border border-slate-700">
                    ...
                    {!canSearch && serverStatus !== 'checking' && (
                      <p className="text-center text-yellow-400 text-sm mt-4">
                        伺服器連線或設定失敗，搜尋功能已暫時停用。請參考下方的錯誤訊息。
                      </p>
                    )}
                </div>
